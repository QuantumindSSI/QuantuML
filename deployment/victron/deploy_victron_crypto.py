...
def export_onnx(model, tokenizer, output_dir: Path, max_length: int = 1024):
...
def create_victron_package(model_dir: str, output_dir: Path, cfg: dict):
    ...
    with open(startup_script, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("# Victron Edge Inference Server - Quantum-Resistant Crypto Analyzer\n")
        f.write('cd "$(dirname "$0")"\n')
        f.write("python3 -m uvicorn api_server_crypto:app --host 0.0.0.0 --port 8000\n")
    os.chmod(startup_script, 0o755)

    service_file = pkg_dir / "crypto-analyzer.service"
    with open(service_file, "w") as f:
        f.write("""[Unit]
Description=Quantum-Resistant Cryptographic Protocol Analyzer
...
ExecStart=/opt/crypto-analyzer/start.sh
Restart=always
RestartSec=10
...
def main():
    parser = argparse.ArgumentParser(description="Deploy Quantum-Resistant Crypto Analyzer to Victron Edge")
    ...
    print("To deploy:")
    print(f"  1. Copy {pkg_dir} to Victron device /opt/crypto-analyzer")
    print(f"  2. Install: sudo cp crypto-analyzer.service /etc/systemd/system/")
    print(f"  3. Enable: sudo systemctl enable --now crypto-analyzer")
